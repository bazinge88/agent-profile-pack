...
description:
  浏览器 CDP 执行层 — 由 anysearch skill 自动路由调用，处理需要真实浏览器环境的网络任务。
  触发场景：需要登录态的网站（小红书、微博、公众号等）、动态渲染页面、视频内容、复杂交互操作。
  注意：此 skill 不应被独立调用，所有联网请求应通过 anysearch skill 统一入口。
metadata:
  author: 一泽Eze
  version: "2.6.0"
---
...
**此 skill 是 anysearch 的底层执行层，由 anysearch 自动路由调用。**

```
用户联网请求 → anysearch skill（统一入口）
                    │
                    ├─ 通用搜索 → anysearch API
                    ├─ 垂直搜索 → anysearch API
                    ├─ 静态页面提取 → anysearch extract
                    │
                    └─ 需要登录态/动态渲染/视频 → web-access（本 skill）
```

**请勿独立调用此 skill。** 所有联网操作应通过 anysearch skill 发起，由 anysearch 根据请求类型自动路由到本 skill。
...
执行搜索任务时，**默认使用 DeepSeek 进行联网搜索**，利用其深度思考和联网搜索能力获取信息：

**默认 AI 搜索：**

| AI 平台 | URL | 特点 |
|---------|-----|------|
| DeepSeek | `https://chat.deepseek.com/` | 深度思考 +联网搜索，免费 |

**DeepSeek 搜索执行流程：**

1. 打开 DeepSeek：`curl -s -X POST --data-raw 'https://chat.deepseek.com/' http://localhost:3456/new`
2. 等待页面加载（约4秒）
3. 点击"专家模式"按钮：
   ```javascript
   const elements = Array.from(document.querySelectorAll("div, span, button"));
   const expertBtn = elements.find(el => el.innerText === "专家模式");
   if (expertBtn) expertBtn.click();
   ```
4. 等待模式切换（约2秒）
5. 点击输入框聚焦：`curl -s -X POST "http://localhost:3456/clickAt?target=ID" -d 'textarea'`
6. 输入搜索内容（关键方法）：
   ```javascript
   // 使用 execCommand 插入文本，可触发 React 事件
   document.execCommand("insertText", false, "搜索关键词");
   ```
7. 发送消息：
   ```javascript
   // 模拟 Enter 键发送
   ...
   ```
8. 等待 AI 回复（约8-15秒），提取内容：`document.body.innerText`

**注意事项：**
- 必须先点击"专家模式"才能获得更好的搜索结果
- `execCommand("insertText")` 是关键，直接设置 `textarea.value` 对 React 无效
- 搜索结果包含 AI 总结和来源链接，信息更全面
